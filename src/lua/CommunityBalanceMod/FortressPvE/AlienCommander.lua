...
if Server then
    ...
    function AlienCommander:ProcessTechTreeAction(techId, pickVec, orientation, worldCoordsSpecified, targetId, shiftDown)
        ...
        if success then
            ...
            elseif techId == kTechId.FortressShiftAbility then 
               soundToPLay = AlienCommander.kWhipFury --TODO doesnt work
            elseif techId == kTechId.FortressShadeAbility then 
                 soundToPLay = AlienCommander.kWhipFury -- TODO doesnt work
    ...
    function AlienCommander:ProcessTechTreeActionForEntity(techNode, position, normal, pickVec, orientation, entity, trace, targetId)
        ...
        if not entity and ( techId == kTechId.ShadeInk or techId == kTechId.HealWave ) then
            ...
            --[[
            -- balance mod TODO doesnt work
            if techId == kTechId.HealWave and not GetHasTech(self, kTechId.CragHive) then 
                className = "FortressCrag"
            elseif techId == kTechId.ShadeInk and not GetHasTech(self, kTechId.ShadeHive) then 
                className = "FortressShade"
            end]]
...
function AlienCommander:GetTechAllowed(techId, techNode)
    ...
    elseif techId == kTechId.SelectShift then
        ...
        allowed = allowed and ( GetHasTech(self, kTechId.ShiftHive) or GetHasTech(self, kTechId.FortressShift) ) -- balance mod. TODO fortresses
    ...
    elseif techId == kTechId.HealWave then
        ...
        allowed = allowed and ( GetHasTech(self, kTechId.CragHive) or GetHasTech(self, kTechId.FortressCrag) )-- balance mod. TODO fortresses
    
    elseif techId == kTechId.ShadeInk then
    
        allowed = GetNearest(self, "Shade") ~= nil

        allowed = allowed and ( GetHasTech(self, kTechId.ShadeHive) or GetHasTech(self, kTechId.FortressShade) ) -- balance mod. TODO fortresses
