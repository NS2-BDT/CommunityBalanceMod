...
local kPurificationMinIntensity = 0.4
local kPurificationCycleTime = 2
...
function PowerPointLightHandler:Init(powerPoint)
    ...
    self.workerTable = {
		...
		[kLightMode.Purification] = PurificationLightWorker():Init(self, "purification"),
    }
...
-- Handles Purification warning.
-- This cycles the light constantly
class 'PurificationLightWorker' (BaseLightWorker)

function PurificationLightWorker:Run()

    PROFILE("PurificationLightWorker:Run")

    local timeOfChange = self.handler.powerPoint:GetTimeOfLightModeChange()
    local time = Shared.GetTime()
    local timePassed = time - timeOfChange

    local scalar = math.cos((timePassed / (kPurificationCycleTime / 2)) * math.pi / 2)
    local minIntensity = kPurificationMinIntensity
    local halfIntensity = (1 - minIntensity) / 2
