...
function ExoWelder:OnPrimaryAttack(player)
    ...
    if self.timeLastWeld + kExoWelderWelderFireDelay < Shared.GetTime() then
    ...
    if not self.timeLastWeldEffect or self.timeLastWeldEffect + kExoWelderWelderEffectRate < Shared.GetTime() then
...
function ExoWelder:PerformWeld(player)
    ...
    if didHit and target and HasMixin(target, "Live") then
        ...
        if GetAreEnemies(player, target) then
            self:DoDamage(kExoWelderWelderDamagePerSecond * kExoWelderWelderFireDelay, target, endPoint, attackDirection)
            success = true
        elseif player:GetTeamNumber() == target:GetTeamNumber() and HasMixin(target, "Weldable") then
            ...
            if target:GetHealthScalar() < 1 then
                ...
                if target:GetCanBeWelded(player) then
                    if target.OnWeldOverride then
                        target:OnWeldOverride(player, kExoWelderWelderFireDelay)
                    else
                        target:AddHealth(self:GetRepairRate(target) * kExoWelderWelderFireDelay)
                ...
                if success then
                    ...
                    player:AddContinuousScore("WeldHealth", addAmount, kExoWelderAmountHealedForPoints, kExoWelderHealScoreAdded)
                    
                    -- weld owner as well
                    player:SetArmor(player:GetArmor() + kExoWelderWelderFireDelay * kExoWelderSelfWeldAmount)
            ...
            if HasMixin(target, "Construct") then
                target:Construct(kExoWelderWelderFireDelay, player)
            end
